...
@end
@implementation StephenLiuTopAlert
...
#pragma mark Stephen - 基础设置
- (void)createAlert{
    //设置提示图
    self.backgroundColor = UIColorFromHex(0x3691d1); //[UIColor colorWithHexString:@"3691D1"];
...
#pragma mark Stephen - 根据外部调用个性化显示
...
- (void)alertWithType:(StephenLiuTopAlertType)type title:(NSString *)title{
    ...
    if (type == StephenLiuTopAlertTypeSuccess) {
        self.pointIMGV.image = [UIImage imageNamed:@"icon_tip_success"];
    }
    if (type == StephenLiuAlertTypeError) {
        self.pointIMGV.image = [UIImage imageNamed:@"icon_tip_fail"];
    }
    
    if (type == StephenLiuTopAlertTypeWarning) {
        self.pointIMGV.image = [UIImage imageNamed:@"icon_tip_warm"];
    }
    
    if (type == StephenLiuAlertTypeMessage) {
        self.pointIMGV.image = [UIImage imageNamed:@"icon_tip_warm"];
    }
}
...
#pragma mark Stephen -  展示提示框
- (void)show{
    UIWindow *window = [UIApplication sharedApplication].keyWindow;
    [window addSubview:self];
}

#pragma mark Stephen - 移除提示框
- (void)removeAlert{
    [UIView transitionWithView:self duration:0.25 options:0 animations:^{
        self.center = CGPointMake([UIScreen mainScreen].bounds.size.width / 2,-32);
    } completion:^(BOOL finished) {
